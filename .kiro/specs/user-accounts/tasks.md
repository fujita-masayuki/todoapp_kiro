...
- [ ] 11. アカウント削除機能の実装
  - ユーザーアカウントとすべての関連データを安全に削除する機能を実装する
  - 削除前の確認プロセスとエラーハンドリングを追加する
  - _要件: 7.1, 7.2, 7.3, 7.4, 7.5_

- [ ] 11.1 バックエンドアカウント削除APIの実装
  - UsersController に DELETE アクションを追加する
  - ユーザー認証確認とアカウント削除処理を実装する
  - 関連するTodoの自動削除（CASCADE）を確認する
  - 削除処理のトランザクション管理を実装する
  - _要件: 7.2, 7.4, 7.5_

- [ ] 11.2 フロントエンドアカウント削除UIの実装
  - AuthContext に deleteAccount メソッドを追加する
  - アカウント削除ボタンをナビゲーションに追加する
  - 削除確認モーダルダイアログを実装する
  - 削除成功時のログアウトとリダイレクト処理を実装する
  - _要件: 7.1, 7.3, 7.5_

- [ ] 11.3 アカウント削除機能のテスト実装
  - バックエンドアカウント削除APIのテストを作成する
  - フロントエンド削除確認UIのテストを作成する
  - 削除後のデータ整合性テストを作成する
  - エラーケースのテストを作成する
  - _要件: 7.1, 7.2, 7.3, 7.4, 7.5_
  ...
  ルール
  開発環境はDockerで動いています。
  README.mdを確認して作業してください。
  また、タスク作業時は必ず日本語で会話を進めてください。